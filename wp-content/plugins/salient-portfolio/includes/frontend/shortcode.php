										...
										<div class="work-info"> 
											...
											if($using_custom_content === 'on') {
												if(!empty($custom_project_link)) {
													echo '<a href="'. esc_attr( $the_project_link ) .'"><span class="screen-reader-text">'.get_the_title().'</span></a>';
												}
								...
								// Project style 2.
									...
									<div class="work-item style-2" data-custom-content="<?php echo esc_attr( $using_custom_content ); ?>">
										...
										<div class="work-info">
											...
											//custom content
											if($using_custom_content == 'on') {
												if(!empty($custom_project_link)) echo '<a href="'. esc_url( $the_project_link ) .'"><span class="screen-reader-text">'.get_the_title().'</span></a>';
												//default
											} else { ?>
												...
												<?php if($lightbox_only != 'true' && $lightbox_only_item !== 'on' ) { ?>
													...
													<a href="<?php echo esc_url( $the_project_link ); ?>"><span class="screen-reader-text"><?php echo get_the_title(); ?></span></a>
								...
								<?php } //project style 2 
									...
									<div class="work-item style-3" data-custom-content="<?php echo esc_attr( $using_custom_content ); ?>" data-text-align="<?php echo esc_attr( $masonry_item_content_pos ); ?>">
										...
										<div class="work-info">
											...
											if($using_custom_content === 'on') {
												if(!empty($custom_project_link)) {
													echo '<a href="'. esc_url( $the_project_link ) .'"><span class="screen-reader-text">'.get_the_title().'</span></a>';
												}
												//default
											} else {
												...
												if($lightbox_only != 'true' && $lightbox_only_item !== 'on' ) { ?>
													...
													<a href="<?php echo esc_url( $the_project_link ) ; ?>"><span class="screen-reader-text">'.get_the_title().'</span></a>
					...
					if ( $the_query->have_posts() ) {
						while ( $the_query->have_posts() ) {
							...
							if($lightbox_only !== 'true') {
								$link_markup = '<a href="' . esc_attr($the_project_link) . '"><span class="screen-reader-text">'.get_the_title().'</span></a>';
							} else {
							...
							if($using_custom_content === 'on') {
								if(!empty($custom_project_link)) {
									echo '<a href="'.$the_project_link.'"><span class="screen-reader-text">'.get_the_title().'</span></a>';
								}
								$recent_projects_content .= '<div class="vert-center"><div class="custom-content">' . do_shortcode($custom_content) . '</div></div>';
								//default
							} else { 
				...
				if($project_style === 'fullscreen_zoom_slider') {
					...
					if ( $the_query->have_posts() ) {
						while ( $the_query->have_posts() ) {
							...
							$recent_projects_content .='<section class="project-slide '.$active_class.'">';
							...
							$recent_projects_content .= '<div class="project-info"><div class="container normal-container">';
							if ( empty($slider_heading_structure) || $slider_heading_structure === 'default' ) {
								$recent_projects_content .= '<h1 class="project-slide__title">'. get_the_title(). '</h1> ';
							} else if ( $slider_heading_structure === 'first_h1' ) {
								if ( $projcount === 0 ) {
									$recent_projects_content .= '<h1 class="project-slide__title">'. get_the_title(). '</h1> ';
								} else {
									$recent_projects_content .= '<h2 class="project-slide__title" data-inherit-heading-family="h1">'. get_the_title(). '</h2> ';
								}
							} else {
								$recent_projects_content .= '<h2 class="project-slide__title">'. get_the_title(). '</h2> ';
							}
							$recent_projects_content .= $fullscreen_slider_excerpt.' <a href="'.esc_attr($the_project_link).'">' . $fullscreen_slider_link_text . '</a></div></div>';
							$recent_projects_content .= '</section>';
