 ...
 */
 ...
 function nectar_add_rbfw_items_to_woocommerce_queries( $q ) {

    // Condição 1: Apenas executa no frontend e para as consultas de produto (não no admin).
    if ( is_admin() ) {
        return;
    }

    // Condição 2: Verifica se é uma página de arquivo do WooCommerce (loja, categoria, tag).
    if ( is_shop() || is_product_category() || is_product_tag() ) {
        
        // Pega os tipos de post existentes na consulta.
        $post_types = $q->get( 'post_type' );

        // Se a consulta for apenas para 'product', adiciona o seu tipo de post personalizado.
        if ( $post_types == 'product' ) {
            $q->set( 'post_type', array( 'product', 'rbfw_item' ) );
        }
    }
}
add_action( 'woocommerce_product_query', 'nectar_add_rbfw_items_to_woocommerce_queries' );


function nectar_remove_visibility_filter_for_custom_items( $tax_query ) {
    
    // Condição 1: Apenas executa no frontend e nas páginas de arquivo do WooCommerce.
    if ( is_admin() || !is_woocommerce() || !( is_shop() || is_product_category() || is_product_tag() ) ) {
        return $tax_query;
    }

    // Condição 2: Procura e remove o filtro de visibilidade de produtos.
    if ( is_array( $tax_query ) ) {
        foreach ( $tax_query as $key => $query_part ) {
            if ( isset( $query_part['taxonomy'] ) && $query_part['taxonomy'] === 'product_visibility' ) {
                unset( $tax_query[ $key ] );
            }
        }
    }
...
add_filter( 'woocommerce_product_query_tax_query', 'nectar_remove_visibility_filter_for_custom_items' );
